...
function parse(intermediateData, databaseType, force) {
...
function getOption(classId, optionType) {
  ...
  parsedData.options.forEach(function (option) {
    if (optionType === option.name
        && (option.entityNames.has(classId) || (option.entityNames.has('*') && !option.excludedNames.has(classId)))) {
      val = option.value;
    }
  });
  return val; // todo it will be nil, throw an exception!
}
