...
function checkNoSQLModeling(passedJdlObject, passedDatabaseType) {
  ...
  if (
    passedJdlObject.getRelationshipQuantity() !== 0 &&
    !DatabaseTypes.isSql(passedDatabaseType) &&
    passedDatabaseType !== DatabaseTypes.MONGODB
  ) {
...
function setSourceAssociationsForClass(relatedRelationships, entityName) {
  for (let i = 0; i < relatedRelationships.from.length; i++) {
    ...
    if (relatedRelationship.type === RelationshipTypes.ONE_TO_ONE) {
      ...
      relationship.otherEntityRelationshipName = _.lowerFirst(
      ...
      if (!relatedRelationship.injectedFieldInTo) {
        relationship.otherEntityRelationshipName = _.lowerFirst(relatedRelationship.from);
        otherSplitField = extractField(relatedRelationship.injectedFieldInTo);
...
function setDestinationAssociationsForClass(relatedRelationships, entityName) {
  for (let i = 0; i < relatedRelationships.to.length; i++) {
    ...
    const relationshipType =
      relatedRelationship.type === RelationshipTypes.ONE_TO_MANY
        ? RelationshipTypes.MANY_TO_ONE
        : relatedRelationship.type;
    const relationship = {
    ...
    if (relatedRelationship.type === RelationshipTypes.ONE_TO_ONE) {
      ...
      relationship.otherEntityRelationshipName =
        _.lowerFirst(otherSplitField.relationshipName) || camelCase(relatedRelationship.from);
    } else if (relatedRelationship.type === RelationshipTypes.ONE_TO_MANY) {
      relatedRelationship.injectedFieldInTo =
        relatedRelationship.injectedFieldInTo || _.lowerFirst(relatedRelationship.from);
      splitField = extractField(relatedRelationship.injectedFieldInTo);
