...
function setValidationsOfField(jdlField, fieldData) {
  ...
  for (let i = 0, validationNames = Object.keys(jdlField.validations); i < validationNames.length; i++) {
    ...
    if (validation.name !== 'required') {
      let name = `fieldValidateRules${_.capitalize(validation.name)}`;
      if (name === 'fieldValidateRulesPattern') {
        fieldData[`${name}Java`] = validation.value;
      }
      fieldData[name] = validation.value;
    }
  }
}
