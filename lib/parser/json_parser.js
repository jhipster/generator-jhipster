...
const RelationshipTypes = require('../core/jhipster/relationship_types').RelationshipTypes;
const UnaryOptions = require('../core/jhipster/unary_options').Options;
...
function getTypeForBlob(fieldTypeBlobContent) {
  if (['image', 'any', 'text'].includes(fieldTypeBlobContent)) {
    return FieldTypes.CommonDBTypes[`${fieldTypeBlobContent.toUpperCase()}_BLOB`];
  }
  return null;
}
...
function getRelationship(relationship, entityName) {
  ...
  if (shouldHandleUserEntity(relationship)) {
    ...
    relationshipConfiguration.otherEntity = configuration.entities[_.upperFirst(relationship.otherEntityName)];
  ...
  if (relationship.relationshipType === 'many-to-one') {
    ...
    relationshipConfiguration.type = RelationshipTypes.MANY_TO_ONE;
  } else if (relationship.relationshipType === 'one-to-one' && relationship.ownerSide === true) {
    relationshipConfiguration.type = RelationshipTypes.ONE_TO_ONE;
  } else if (relationship.relationshipType === 'many-to-many' && relationship.ownerSide === true) {
    relationshipConfiguration.type = RelationshipTypes.MANY_TO_MANY;
  }
...
function setUpRelationshipConfiguration(relationship, entityName, relationshipConfiguration) {
  relationshipConfiguration.toJdlEntity = configuration.jdl.entities[_.upperFirst(relationship.otherEntityName)];
...
function getDefaultRelationship(relationship, entityName, relationshipConfiguration) {
  return new JDLRelationship({ // eslint-disable-line consistent-return
    ...
    to: relationshipConfiguration.toJdlEntity,
...
function addEntityOptionsToJDL(entity, entityName) {
  if (entity.fluentMethods === false) {
    addUnaryOptionToJDL(UnaryOptions.NO_FLUENT_METHOD, entityName);
  }
  ...
  if (entity.angularJSSuffix) {
    addBinaryOptionToJDL(BinaryOptions.ANGULAR_SUFFIX, entity.angularJSSuffix, entityName);
  }
  // microservice in BinaryOptions, microserviceName in Json
  if (entity.microserviceName !== undefined) {
    addBinaryOptionToJDL(BinaryOptions.MICROSERVICE, entity.microserviceName, entityName);
  }
  if (entity.jpaMetamodelFiltering === true) {
    addUnaryOptionToJDL(UnaryOptions.FILTER, entityName);
  }
  if (entity.clientRootFolder) {
    addBinaryOptionToJDL(BinaryOptions.CLIENT_ROOT_FOLDER, entity.clientRootFolder, entityName);
  }
}
