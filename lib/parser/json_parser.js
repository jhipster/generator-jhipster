...
function addRelationshipsToJDL(jdl, entities, skipUserManagement) {
  for (let i = 0, entityNames = Object.keys(entities); i < entityNames.length; i++) {
    ...
    for (let relationship of entity.relationships) {
      addRelationshipToJDL(relationship, jdl, entities, skipUserManagement, entityName);
    }
  }
}

function addRelationshipToJDL(relationship, jdl, entities, skipUserManagement, entityName) {
  ...
  var type = getRelationshipType(relationship, injectedFieldInTo);
...
function addEntityOptionsToJDL(jdl, entity, entityName) {
  ...
  if (entity.angularJSSuffix !== undefined) {
    jdl.addOption( new JDLBinaryOption({
