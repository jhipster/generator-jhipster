 ...
 */
function parse(passedDocument, passedDatabaseType, applicationType, applicationName, generationVersion) {
  ...
  fillApplications(generationVersion);
  fillEnums();
  fillClassesAndFields(passedDatabaseType);
  fillAssociations();
  fillOptions(passedDatabaseType, applicationType, applicationName);
  return jdlObject;
}
...
function fillApplications(generatorVersion) {
  for (let i = 0; i < document.applications.length; i++) {
    ...
    jdlObject.addApplication(new JDLApplication(document.applications[i]));
  }
}
