...
const _ = require('lodash'),
    ...
    isReservedEntityName = require('../core/jhipster/reserved_keywords').isReservedEntityName,
...
function fillClassesAndFields(passedDatabaseType) {
  for (let i = 0; i < document.entities.length; i++) {
    ...
    if (isReservedEntityName(entity.name)) {
      throw new buildException(exceptions.IllegalName, `The name '${entity.name}' is reserved keyword and can not be used as entity name.`);
    }
    let tableName = entity.tableName || entity.name;
    if (isReservedTableName(tableName, passedDatabaseType)) {
      throw new buildException(exceptions.IllegalName, `The name '${tableName}' is reserved DB keyword and can not be used as table name.`);
    }
    jdlObject.addEntity({
      name: entity.name,
      tableName: tableName,
