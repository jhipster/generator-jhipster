 ...
 * @deprecated Use DocumentParser::parseFromConfigurationObject instead
 */
function parse(document, databaseType, applicationType, applicationName, generatorVersion) {
  DeprecationUtils.displayMethodDeprecationMessage({
    deprecatedMethod: 'DocumentParser::parse',
    preferredMethod: 'DocumentParser::parseFromConfigurationObject'
  });
...
function fillApplications() {
  for (let i = 0; i < configuration.document.applications.length; i++) {
    configuration.document.applications[i].config.jhipsterVersion = configuration.generatorVersion;
    jdlObject.addApplication(new JDLApplication(configuration.document.applications[i]));
  }
}
...
function fillBinaryOptions() {
  _.forEach(BinaryOptions.BINARY_OPTIONS, (optionValue) => {
    _.forEach(configuration.document[optionValue], (documentOptionValue, documentOptionKey) => {
      ...
      if (configuration.applicationType === ApplicationTypes.MICROSERVICE
        && optionValue === BinaryOptions.BINARY_OPTIONS.CLIENT_ROOT_FOLDER) {
