...
function isTableNameReservedForEntity(entityTableName, entityName) {
  const applications = applicationsPerEntityName[entityName];
  if (applications.length === 0) {
    return false;
  }
  return applications.some(application => isReservedTableName(entityTableName, application.config.databaseType));
}
