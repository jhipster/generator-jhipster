 ...
 */
import { type CstNode, CstParser, type IParserConfig, type TokenType } from 'chevrotain';
...
export default class JDLParser extends CstParser {
  ...
  constructor(tokens: Record<string, TokenType>, options: IParserConfig = {}) {
    super(tokens, { recoveryEnabled: false, nodeLocationTracking: 'full', ...options });
    this.tokens = tokens;
  }
  ...
  prog(): CstNode {
    this.RULE('prog', () => {
      this.MANY(() => {
        this.OR({
          // Commit to a declaration's structural keyword before inspecting its required
          // name, so recovery can repair a malformed declaration and retain the next one.
          MAX_LOOKAHEAD: 1,
          IGNORE_AMBIGUITIES: true,
          DEF: [
            {
              GATE: () =>
                this.LA(1).tokenType !== this.tokens.JAVADOC || [this.tokens.ENTITY, this.tokens.AT].includes(this.LA(2).tokenType),
              ALT: () => this.SUBRULE(this.entityDeclaration),
            },
            { ALT: () => this.SUBRULE(this.relationDeclaration) },
            {
              GATE: () => this.LA(1).tokenType !== this.tokens.JAVADOC || this.LA(2).tokenType === this.tokens.ENUM,
  ...
  validation(): CstNode {
    this.RULE('validation', () => {
      this.OR([
        { ALT: () => this.CONSUME(this.tokens.FLAG_VALIDATION) },
  ...
  minMaxValidation(): CstNode {
    this.RULE('minMaxValidation', () => {
      // Numeric validation names come from the caller's definitions.
      this.CONSUME(this.tokens.NUMBER_VALIDATION, { LABEL: 'MIN_MAX_KEYWORD' });
      this.CONSUME(this.tokens.LPAREN);
      this.OR([
  ...
  pattern(): CstNode {
    this.RULE('pattern', () => {
      this.CONSUME(this.tokens.PATTERN_VALIDATION, { LABEL: 'PATTERN' });
      this.CONSUME(this.tokens.LPAREN);
  ...
  relationshipType(): CstNode {
    this.RULE('relationshipType', () => {
      this.CONSUME(this.tokens.NAME, { LABEL: 'RELATIONSHIP_TYPE' });
    });
    return noopCst;
  }
  ...
  relationshipSide(): CstNode {
    this.RULE('relationshipSide', () => {
      ...
      this.OPTION(() => {
        ...
        this.OPTION2(() => {
          this.CONSUME(this.tokens.FLAG_VALIDATION, { LABEL: 'REQUIRED' });
        });
        this.CONSUME(this.tokens.RCURLY);
      });
    });
  ...
  relationshipOption(): CstNode {
    this.RULE('relationshipOption', () => {
      this.CONSUME(this.tokens.NAME, { LABEL: 'RELATIONSHIP_OPTION' });
    });
    return noopCst;
  }
  ...
  optionDeclaration(): CstNode {
    this.RULE('optionDeclaration', () => {
      ...
      this.OPTION(() => {
        ...
        this.OR([
          { ALT: () => this.CONSUME2(this.tokens.NAME, { LABEL: 'method' }) },
          { ALT: () => this.CONSUME(this.tokens.STRING, { LABEL: 'methodPath' }) },
        ]);
      });
      this.OPTION2(() => {
        this.SUBRULE(this.exclusion);
  ...
  deploymentConfigDeclaration(): CstNode {
    this.RULE('deploymentConfigDeclaration', () => {
      this.CONSUME(this.tokens.NAME, { LABEL: 'DEPLOYMENT_KEY' });
  ...
  applicationConfigDeclaration(): CstNode {
    this.RULE('applicationConfigDeclaration', () => {
      this.CONSUME(this.tokens.NAME, { LABEL: 'CONFIG_KEY' });
