 ...
 */
export const createRuntime = (definitions: JDLDefinitions): JDLRuntime => {
  const {
  ...
  const getJDLTokens = () => {
    if (!jdlTokens) {
      jdlTokens = buildTokens({ namePattern, validations });
    }
    return jdlTokens;
  };
  ...
  return {
    ...
    get parser(): JDLParser {
      if (!parser) {
        parser = new JDLParser(this.tokens, { recoveryEnabled: true });
        parser.parse();
