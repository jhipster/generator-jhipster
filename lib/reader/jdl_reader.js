...
function parseFromFiles(files) {
  ...
  return parse(files.length === 1 ? FileReader.readFile(files[0]) : aggregateFiles(files));
}
...
/**
 * Parsed the stringified JDL content and returns an intermediate object from the Chevrotain parser.
