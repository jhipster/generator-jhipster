...
function parseFromDir(dir) {
  if (!dir) {
    throw new Error('The app directory must be passed.');
  }
  if (!FileUtils.doesDirectoryExist(dir)) {
    throw new Error(`The passed directory '${dir}' must exist and must be a directory.`);
  }
  const entityDir = `${dir}/.jhipster`;
  if (!FileUtils.doesDirectoryExist(entityDir)) {
    throw new Error(`'${entityDir}' must be a directory.`);
  }
  const entities = {};
