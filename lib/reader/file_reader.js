...
DeprecationUtils.displayObjectDeprecationMessage({
  deprecatedObject: 'FileReader',
  preferredObject: 'JDLReader'
});
...
function readFile(path) {
  if (!path) {
    throw new Error('The passed file must not be nil.');
  }
  if (!FileUtils.doesFileExist(path)) {
    throw new Error(`The passed file '${path}' must exist and must not be a directory.`);
  }
