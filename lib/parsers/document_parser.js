 ...
 * @param {Boolean} configuration.skippedUserManagement - Whether user management is skipped
...
function convertAnnotationsToOptions(annotations) {
  ...
  annotations.forEach(annotation => {
    ...
    if (annotation.optionName in result) {
      const oldValue = result[annotation.optionName];
      if (Array.isArray(oldValue)) {
        if (!oldValue.includes(value)) {
          oldValue.push(value);
        }
      } else if (value !== oldValue) {
        result[annotation.optionName] = [oldValue, value];
      }
    } else {
