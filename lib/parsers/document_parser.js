...
const { convertValidation } = require('../converters/parsedJDLToJDLObject/validation_converter');
...
function getValidations(field) {
  ...
  for (let i = 0; i < field.validations.length; i++) {
    ...
    validations[validation.key] = convertValidation(validation, getConstantValueFromConstantName);
  }
  return validations;
}
