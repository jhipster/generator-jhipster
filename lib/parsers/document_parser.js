...
function fillAssociations() {
  for (let i = 0; i < configuration.document.relationships.length; i++) {
    ...
    const options = convertAnnotationsToOptions(relationship.annotations);
...
function convertAnnotationsToOptions(annotations) {
  ...
  annotations.forEach(o => {
    const value = o.method ? o.method : true;
    if (o.option in result) {
      const oldValue = result[o.option];
      if (Array.isArray(oldValue)) {
        if (value !== oldValue) {
          oldValue.push(value);
        }
      } else if (value !== oldValue) {
        result[o.option] = [oldValue, value];
      }
    } else {
      result[o.option] = value;
    }
  });
  return result;
}
