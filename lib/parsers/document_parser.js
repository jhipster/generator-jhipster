...
const { convertBinaryOptions } = require('../converters/parsedJDLToJDLObject/binary_option_converter');
...
function fillUnaryOptions() {
  const skipClientOption = configuration.document.options.skipClient;
  const skipServerOption = configuration.document.options.skipServer;
  const noFluentMethodOption = configuration.document.options.noFluentMethod;
  const filterOption = configuration.document.options.filter;
  const readOnlyOption = configuration.document.options.readOnly;
...
function fillBinaryOptions() {
  ...
  const convertedBinaryOptions = convertBinaryOptions(configuration.document.options, getBinaryOptionEntityList);
  convertedBinaryOptions.forEach(convertedBinaryOption => {
    jdlObject.addOption(convertedBinaryOption);
  });
}

function getBinaryOptionEntityList(name, value) {
  const { list, excluded } = configuration.document.options[name][value];
  return {
    entityList: list,
    excludedEntityList: excluded
  };
}
