...
function fillBinaryOptions() {
  ...
  const convertedBinaryOptions = convertBinaryOptions(configuration.document, getBinaryOptionEntityList);
...
function getBinaryOptionEntityList(name, value) {
  const { list, excluded } = configuration.document[name][value];
  return {
    entityList: list,
