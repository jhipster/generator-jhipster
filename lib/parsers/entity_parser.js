  ...
  option.entityNames.forEach(entityName => {
    switch (option.name) {
      ...
      case ANGULAR_SUFFIX:
        ...
        break;
      case SEARCH:
        entities[entityName].searchEngine = option.value;
        break;
      case FILTER:
        ...
        break;
      default:
        entities[entityName][option.name] = option.value ? option.value : true;
    }
  });
...
function setFieldsOfEntity(entityName) {
  for (let i = 0, fieldNames = Object.keys(jdlObject.getEntity(entityName).fields); i < fieldNames.length; i++) {
    ...
    setOptionsOfField(jdlField, fieldData);
...
function setOptionsOfField(jdlField, fieldData) {
  if (Object.entries(jdlField.options).length !== 0) {
    fieldData.options = {};
    Object.entries(jdlField.options).forEach(option => {
      fieldData.options[option[0]] = option[1];
    });
  }
}
...
function setSourceAssociationsForClass(relatedRelationships, entityName) {
  relatedRelationships.from.forEach(relatedRelationship => {
    ...
    setOptionsOfRelationship(relatedRelationship, relationship);
...
function setOptionsOfRelationship(relatedRelationship, relationship) {
  if (relatedRelationship.options) {
    Object.entries(relatedRelationship.options).forEach(option => {
      if (option[0] === JPA_DERIVED_IDENTIFIER) {
        relationship.useJPADerivedIdentifier = option[1];
      } else {
        if (!relationship.options) {
          relationship.options = {};
        }
        relationship.options[option[0]] = option[1];
      }
    });
  }
}
