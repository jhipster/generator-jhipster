...
function parse(args) {
  ...
  if (merged.applicationType !== GATEWAY) {
    checkNoSQLModeling(args.baseName, merged.jdlObject, args.databaseType);
  }
  init(merged);
  initializeEntities();
  setOptions();
...
function checkNoSQLModeling(baseName, passedJdlObject, passedDatabaseType) {
  if (passedDatabaseType === NO) {
    return;
  }
  if (passedJdlObject.getRelationshipQuantity(baseName) !== 0 && passedDatabaseType === CASSANDRA) {
