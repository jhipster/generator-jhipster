...
function checkBaseNameAgainstApplicationType(jdlApplication) {
  if (
    /_/.test(jdlApplication.config.baseName) &&
    (jdlApplication.config.applicationType === UAA || jdlApplication.config.applicationType === MICROSERVICE)
  ) {
    throw new Error(
...
function stripCustomOptions(jdlApplication) {
  ...
  delete configCopy.creationTimestamp;
  return configCopy;
}
