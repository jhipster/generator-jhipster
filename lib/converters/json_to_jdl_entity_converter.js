...
const { FILTER, NO_FLUENT_METHOD, READ_ONLY } = require('../core/jhipster/unary_options');
const {
  ANGULAR_SUFFIX,
  CLIENT_ROOT_FOLDER,
  DTO,
...
function addEntity(entity, entityName) {
  if (entityName === USER_ENTITY_NAME && !skippedUserManagement) {
    throw new Error(`User entity name is reserved if ${ApplicationOptions.names.SKIP_USER_MANAGEMENT} is not set.`);
  }
