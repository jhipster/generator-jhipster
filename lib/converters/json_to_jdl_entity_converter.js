...
function convertEntitiesToJDL(entities, jdl) {
  ...
  init(entities, jdl || new ValidatedJDLObject());
  addEntities();
...
function getTypeForBlob(fieldTypeBlobContent) {
  ...
  throw new Error(`Unrecognised blob type: '${fieldTypeBlobContent}'`);
}
    ...
    if (field.fieldValues !== undefined) {
      configuration.jdl.addEnum(
        new JDLEnum({
          ...
          values: field.fieldValues.split(',').map(fieldValue => {
...
function getDefaultRelationship(relationship, entityName, relationshipConfiguration) {
  const jdlRelationship = new JDLRelationship({
  ...
  if (relationship.options) {
    jdlRelationship.options = relationship.options;
  }
  if (relationship.useJPADerivedIdentifier) {
    jdlRelationship.options[JPA_DERIVED_IDENTIFIER] = true;
  }
