...
const BasicEntityConverter = require('./jdl_to_json_basic_entity_converter');
...
function setOptions(entitiesForEachApplication) {
  ...
  jdlObject.forEachApplication(jdlApplication => {
    const convertedOptionContents = OptionConverter.convert(jdlApplication);
...
function formatEntitiesForEachApplication(entitiesForEachApplication) {
  entitiesForEachApplication.forEach((entities, applicationName) => {
    entitiesForEachApplication.set(applicationName, Object.values(entities));
  });
}
