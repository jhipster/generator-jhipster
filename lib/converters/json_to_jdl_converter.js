...
function convertToJDL(directory = '.') {
  if (doesFileExist(path.join(directory, '.yo-rc.json'))) {
    exportSingleJHipsterApplication(directory);
    return;
  }
  exportMultipleJHipsterApplications(directory);
}

function exportMultipleJHipsterApplications(directory) {
  const subDirectories = getSubdirectories(directory);
  subDirectories.forEach(subDirectory => {
    exportSingleJHipsterApplication(path.join(directory, subDirectory));
  });
}

function exportSingleJHipsterApplication(directory) {
  ...
  if (!doesDirectoryExist(path.join(directory, '.jhipster'))) {
    exportJDLObject(jdlApplication, path.join(directory, 'app.jdl'));
    return;
  }
