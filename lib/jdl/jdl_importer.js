...
class JDLImporter {
  ...
  import() {
    ...
    if (jdlObject.getApplicationQuantity() === 0) {
      const jsonEntities = getJSONEntities(jdlObject, this.configuration);
      this.importState.exportedEntities = exportJSONEntities(jsonEntities, this.configuration);
    } else {
      this.importState.exportedApplications = JHipsterApplicationExporter.exportApplications(jdlObject.applications);
      if (jdlObject.getEntityQuantity() !== 0) {
        Object.keys(jdlObject.applications).forEach((applicationName) => {
          ...
          const jsonEntities = getJSONEntities(jdlObject, jdlApplication.config);
          this.importState.exportedEntities = this.importState.exportedEntities.concat(
            exportJSONEntitiesInApplications(jsonEntities, this.configuration, jdlObject.applications)
          );
        });
      }
    }
