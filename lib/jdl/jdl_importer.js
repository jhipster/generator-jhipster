...
function setupConfiguration(configuration) {
  configuration.creationTimestampConfig = new Date();
  // We don't need seconds precision, discard it.
  configuration.creationTimestampConfig.setSeconds(0);
  if (configuration.creationTimestamp) {
    const parsedTimestamp = Date.parse(configuration.creationTimestamp);
    if (parsedTimestamp) {
      configuration.creationTimestampConfig = new Date(parsedTimestamp);
    }
  }
}
...
function getJDLObject(parsedJDLContent, configuration) {
  return DocumentParser.parseFromConfigurationObject({
    ...
    creationTimestamp: configuration.creationTimestampConfig.getTime()
  });
}
...
function importOneApplicationAndEntities(jdlObject, configuration) {
  ...
  if (jdlObject.getEntityQuantity() !== 0) {
    const jsonEntities = getJSONEntitiesFromApplicationConfig(jdlObject, application.getConfig());
...
function getJSONEntitiesFromApplicationConfig(jdlObject, applicationConfig) {
  return EntityParser.parse({
    jdlObject,
    baseName: applicationConfig.getOptionValue('baseName'),
    applicationType: applicationConfig.getOptionValue('applicationType'),
    databaseType: applicationConfig.getOptionValue('databaseType'),
    creationTimestamp: applicationConfig.getOptionValue('creationTimestamp')
  });
}
...
function getJSONEntities(jdlObject, configuration) {
  return EntityParser.parse({
    ...
    applicationType: configuration.applicationType,
    databaseType: configuration.databaseType,
    creationTimestamp: configuration.creationTimestampConfig
  });
}
