...
/**
 * This class' purpose is to import one, or several JDL files/content, and exporters them to JSON (entities & applications).
...
class JDLImporter {
   ...
   * @param jdlString a string having JDL content
   */
  constructor(files, configuration, jdlString = '') {
    if (!files && !jdlString) {
      throw new Error('JDL files or content must be passed so as to be imported.');
    }
    if (jdlString) {
      this.jdlString = jdlString;
    } else {
      this.files = files;
    }
   ...
   */
  import() {
    const parsedJDLContent = this.jdlString ? JDLReader.parseFromContent(this.jdlString) : parseFiles(this.files);
