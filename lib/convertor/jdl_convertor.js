...
function createEntities() {
  ...
  Object.keys(parsedData.entities).forEach(function (entityName) {
     ...
     */
    if (parsedData[entityName].name.toLowerCase() === USER.toLowerCase()) {
      console.warn(
          chalk.yellow(
...
function initializeEntities() {
  ...
  Object.keys(parsedData.entities).forEach(function (classId, index) {
    var initializedEntity = {
      ...
      /*dto: getDTO(classId),
      pagination: parsedData[classId].pagination,
...
/*function getDTO(classId) {
  parsedData.options.forEach(function (option) {
    if(BinaryOptions.BINARY_OPTIONS.DTO === option.name)
  });
}*/
/**
...
function readJSON(classesToRead) {
  ...
  classesToRead.forEach(function (classToRead) {
    var file = '.jhipster/' + parsedData[classToRead].name + '.json';
