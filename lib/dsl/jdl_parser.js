...
module.exports = class JDLParser extends Parser {
  ...
  // the ".input" setter.
  constructor() {
    super([], LexerTokens, { outputCst: true });
    ...
    $.RULE('entityDeclaration', () => {
      $.MANY(() => {
        $.OR([
          { ALT: () => $.SUBRULE($.simpleAnnotationDeclaration) },
          { ALT: () => $.SUBRULE($.complexAnnotationDeclaration) }
        ]);
      });
    ...
    $.RULE('simpleAnnotationDeclaration', () => {
      $.CONSUME(LexerTokens.AT);
      $.OR([
        { ALT: () => $.CONSUME(LexerTokens.SKIP_CLIENT, { LABEL: 'option' }) },
        { ALT: () => $.CONSUME(LexerTokens.SKIP_SERVER, { LABEL: 'option' }) },
        { ALT: () => $.CONSUME(LexerTokens.FILTER, { LABEL: 'option' }) },
        { ALT: () => $.CONSUME(LexerTokens.NO_FLUENT_METHOD, { LABEL: 'option' }) }
      ]);
    });

    $.RULE('complexAnnotationDeclaration', () => {
      $.CONSUME(LexerTokens.AT);
      $.OR([
        { ALT: () => $.CONSUME(LexerTokens.DTO, { LABEL: 'option' }) },
        { ALT: () => $.CONSUME(LexerTokens.PAGINATE, { LABEL: 'option' }) },
        { ALT: () => $.CONSUME(LexerTokens.SERVICE, { LABEL: 'option' }) },
        { ALT: () => $.CONSUME(LexerTokens.MICROSERVICE, { LABEL: 'option' }) },
        { ALT: () => $.CONSUME(LexerTokens.SEARCH, { LABEL: 'option' }) },
        { ALT: () => $.CONSUME(LexerTokens.ANGULAR_SUFFIX, { LABEL: 'option' }) },
        { ALT: () => $.CONSUME(LexerTokens.CLIENT_ROOT_FOLDER, { LABEL: 'option' }) }
      ]);
      $.CONSUME(LexerTokens.LPAREN);
      $.CONSUME(LexerTokens.NAME, { LABEL: 'value' });
