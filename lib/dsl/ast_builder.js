...
class JDLAstBuilderVisitor extends BaseJDLCSTVisitor {
  ...
  prog(context) {
    ...
    _.forEach(optionContextToAstMapping, (astName, contextName) => {
  ...
  entityDeclaration(context) {
    ...
    let javadoc = null;
    if (context.COMMENT) {
      javadoc = trimComment(context.COMMENT[0].image);
    }
    ...
    return {
      annotations, name, tableName, body, javadoc
    };
  }
  ...
  entityBody(context) {
    return _.map(context.fieldDeclaration, element => this.visit(element));
  }

  fieldDeclaration(context) {
    // filter actual comment as the comment rule may be empty
    const comment = context.COMMENT ? trimComment(context.COMMENT[0].image) : null;
    ...
    return {
      ...
      validations: context.validation
        ? _.map(context.validation, element => this.visit(element))
        : [],
      javadoc: comment
    };
  }
  ...
  minMaxValidation(context) {
    ...
    const validationAst = {
      ...
      value: isIdentifier
        ? context.NAME[0].image
        : parseInt(context.INTEGER[0].image, 10)
    };
  ...
  comment(context) {
    if (context.COMMENT) {
      return trimComment(context.COMMENT[0].image);
    }
  ...
  applicationSubDeclaration(context) {
    ...
    if (context.applicationSubConfig) {
      ...
      applicationSubDeclaration.config = this.visit(_.last(context.applicationSubConfig));
    }

    if (context.applicationSubEntities) {
      // Apparently the pegjs grammar only returned the last entities
      applicationSubDeclaration.entities = this.visit(_.last(context.applicationSubEntities));
    }
  ...
  applicationSubConfig(context) {
    ...
    if (context.applicationConfigDeclaration) {
      const configProps = context.applicationConfigDeclaration.map(
        this.visit,
        this
      );
