...
class JDLAstBuilderVisitor extends BaseJDLCSTVisitor {
  ...
  prog(ctx) {
    ...
    _.forEach(optionContextToAstMapping, (astName, ctxName) => {
      if (!ctx[ctxName]) {
        return;
      }
      ctx[ctxName]
  ...
  entityDeclaration(ctx) {
    const annotations = [];
    if (ctx.simpleAnnotationDeclaration) {
      ctx.simpleAnnotationDeclaration.forEach((ctxObject) => {
        annotations.push(this.visit(ctxObject));
      });
    }
    if (ctx.complexAnnotationDeclaration) {
      ctx.complexAnnotationDeclaration.forEach((ctxObject) => {
        annotations.push(this.visit(ctxObject));
      });
    }
  ...
  simpleAnnotationDeclaration(ctx) {
    return { option: ctx.option[0].image, type: 'UNARY' };
  }

  complexAnnotationDeclaration(ctx) {
    return { option: ctx.option[0].image, method: ctx.value[0].image, type: 'BINARY' };
  }
