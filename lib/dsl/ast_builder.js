...
class JDLAstBuilderVisitor extends BaseJDLCSTVisitor {
  ...
  prog(context) {
    ...
    Object.keys(optionContextToAstMapping).forEach(contextName => {
      ...
      context[contextName].map(this.visit, this).forEach(astNewItem => {
        ...
        astResult.list = Array.from(new Set(astResult.list.concat(astNewItem.list)));

        if (astNewItem.excluded) {
          astResult.excluded = Array.from(new Set(astResult.excluded.concat(astNewItem.excluded)));
        }
      });
    });
  ...
  relationshipBody(context) {
    ...
    if (context.relationshipOptions) {
      this.visit(context.relationshipOptions).forEach(o => annotations.push(o));
    }
  ...
  relationshipOption(context) {
    if (context.JPA_DERIVED_IDENTIFIER) {
      return { option: 'useJPADerivedIdentifier', type: 'UNARY' };
    }
  ...
  filterDef(context) {
    let entityList = [];
    if (context.NAME) {
      const value = context.NAME.map(nameToken => nameToken.image, this);
