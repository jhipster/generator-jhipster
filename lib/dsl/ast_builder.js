...
class JDLAstBuilderVisitor extends BaseJDLCSTVisitor {
  ...
  prog(context) {
    ...
    const optionContextToAstMapping = {
      ...
      searchEngineDeclaration: 'search',
      angularSuffixDeclaration: 'angularSuffix',
      noFluentMethod: 'noFluentMethod',
      readOnlyDeclaration: 'readOnly',
      filterDeclaration: 'filter',
      clientRootFolderDeclaration: 'clientRootFolder',
      skipClientDeclaration: 'skipClient',
      skipServerDeclaration: 'skipServer',
      filter: 'filter'
    };

    Object.keys(optionContextToAstMapping).forEach(optionDeclarationType => {
      const optionNameInTheAst = optionContextToAstMapping[optionDeclarationType];
      if (!context[optionDeclarationType]) {
        return;
      }
      context[optionDeclarationType].map(this.visit, this).forEach(newOptionToAddInTheAst => {
        let astResult;
        if (newOptionToAddInTheAst.name) {
          if (!ast.options[optionNameInTheAst][newOptionToAddInTheAst.name]) {
            astResult = { list: newOptionToAddInTheAst.list, excluded: [] };
            ast.options[optionNameInTheAst][newOptionToAddInTheAst.name] = astResult;
          } else {
            astResult = ast.options[optionNameInTheAst][newOptionToAddInTheAst.name];
          }
        } else {
          astResult = ast.options[optionNameInTheAst];
        }

        astResult.list = Array.from(new Set(astResult.list.concat(newOptionToAddInTheAst.list)));

        if (newOptionToAddInTheAst.excluded) {
          astResult.excluded = Array.from(new Set(astResult.excluded.concat(newOptionToAddInTheAst.excluded)));
        }
      });
    });
  ...
  relationshipOption(context) {
    if (context.JPA_DERIVED_IDENTIFIER) {
      return { option: 'jpaDerivedIdentifier', type: 'UNARY' };
    }
  ...
  entityList(context) {
    ...
    return Array.from(new Set(entityList));
  }
  ...
  skipClientDeclaration(context) {
    return extractListExcluded(context, this);
  }

  skipServerDeclaration(context) {
    return extractListExcluded(context, this);
  }
...
function extractNameListExcluded(context, visitor) {
  ...
  const allButLast = _.dropRight(entityList);
  ...
  return { name: last, list: allButLast, excluded: exclusion };
}
