...
const configPropsValidations = {
  ...
  LANGUAGES: { type: 'list', pattern: ALPHABETIC_LOWER, msg: 'languages property' },
  MESSAGE_BROKER:
    {
      type: 'NAME',
      pattern: ALPHANUMERIC,
...
class JDLSyntaxValidatorVisitor extends BaseJDLCSTVisitorWithDefaults {
  ...
  checkNameSyntax(token, expectedPattern, errorMessagePrefix) {
    if (!expectedPattern.test(token.image)) {
      this.errors.push({
        message: `The ${errorMessagePrefix} name must match: ${trimAnchors(
  ...
  constantDeclaration(ctx) {
    ...
    this.checkNameSyntax(ctx.NAME[0], CONSTANT_PATTERN, 'constant');
  }

  entityDeclaration(ctx) {
    super.entityDeclaration(ctx);
    this.checkNameSyntax(ctx.NAME[0], ENTITY_NAME_PATTERN, 'entity');
  }
  ...
  type(ctx) {
    super.type(ctx);
    this.checkNameSyntax(ctx.NAME[0], TYPE_NAME_PATTERN, 'typeName');
  }
  ...
  enumDeclaration(ctx) {
    ...
    this.checkNameSyntax(ctx.NAME[0], ENUM_NAME_PATTERN, 'enum');
  }
  ...
  entityList(ctx) {
    ...
    if (ctx.Method) {
      this.checkNameSyntax(ctx.Method[0], METHOD_NAME_PATTERN, 'method');
    }
  }
