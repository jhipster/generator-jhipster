...
function parseFromDir(dir) {
  ...
  files.forEach(file => {
    if (file.endsWith('.json')) {
      ...
      try {
        entities.set(entityName, JSONFileReader.readEntityJSON(`${entityDir}/${file}`));
      } catch (error) {
        // Not an entity file, not adding
      }
    }
  });
  const jdlObject = convertServerOptionsToJDL(
    JSONFileReader.readEntityJSON(`${dir}/.yo-rc.json`)['generator-jhipster']
  );
