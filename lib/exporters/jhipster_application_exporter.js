...
function setUpApplicationStructure(application) {
  ...
  const configObject = application.getConfig();
  ...
  // applicationToExport[GENERATOR_NAME].testFrameworks = configObject.getOptionValue('testFrameworks');
  // applicationToExport[GENERATOR_NAME].languages = configObject.getOptionValue('languages');
  // applicationToExport[GENERATOR_NAME].otherModules = configObject.getOptionValue('otherModules');
  if (configObject.hasOption('creationTimestamp')) {
    applicationToExport[GENERATOR_NAME].creationTimestamp = configObject.getOptionValue('creationTimestamp');
  }
...
function getApplicationConfig(application) {
  const result = {};
  const configObject = application.getConfig();
  configObject.forEachOption(option => {
...
//     application[GENERATOR_NAME].otherModules = Array.from(application[GENERATOR_NAME].otherModules);
//   }
//   return application;
// }
