...
const { doesFileExist } = require('../utils/file_utils');
const { GENERATOR_NAME, createFolderIfItDoesNotExist, writeConfigFile } = require('./export_utils');
...
function setUpApplicationStructure(application) {
  ...
  applicationToExport[GENERATOR_NAME].otherModules = application.config.otherModules;
  if (application.config.creationTimestamp) {
    applicationToExport[GENERATOR_NAME].creationTimestamp = application.config.creationTimestamp;
  }
