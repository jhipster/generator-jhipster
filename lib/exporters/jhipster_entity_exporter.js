    ...
    exportedEntities = exportedEntities.concat(
      exportEntities({
        ...
        application: {
          ...
          name: applicationConfig.getOptionValue('baseName'),
          type: applicationConfig.getOptionValue('applicationType')
        }
      })
    );
  });
  ...
  for (let i = 0, entityNames = Object.keys(configuration.entities); i < entityNames.length; i++) {
    ...
    if (
      ...
      areJHipsterEntitiesEqual(
        ...
        updateEntityToGenerateWithExistingOne(filePath, configuration.entities[entityName])
      )
    ) {
