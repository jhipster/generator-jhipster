...
function writeEntities(subFolder) {
  configuration.entities.forEach(entity => {
    ...
    const updatedEntity = updateEntityToGenerateWithExistingOne(filePath, entity);
    fs.writeFileSync(filePath, JSON.stringify(updatedEntity, null, 2).concat('\n'));
  });
}
...
function filterOutUnchangedEntities() {
  return configuration.entities.filter(entity => {
    const filePath = toFilePath(entity.name);
    return !(
      FileUtils.doesFileExist(filePath) &&
      areJHipsterEntitiesEqual(readJSONFile(filePath), updateEntityToGenerateWithExistingOne(filePath, entity))
    );
  });
}
...
function filterOutEntitiesByMicroservice() {
  return configuration.entities.filter(entity => {
    return !(
      entity.microserviceName && entity.microserviceName.toLowerCase() !== configuration.application.name.toLowerCase()
    );
  });
}
