...
const merge = require('../utils/object_utils').merge,
    ...
    isReserved = require('./jhipster/reserved_keywords').isReserved,
...
class JDLEntity {
  constructor(args) {
    ...
    if (isReserved(merged.name, 'JHIPSTER')) {
      throw new buildException(exceptions.IllegalName, `The name '${merged.name}' is reserved JHipster keyword and can not be used.`);
    }
    if (isReserved(merged.name, 'JAVA')) {
      throw new buildException(exceptions.IllegalName, `The name '${merged.name}' is reserved Java keyword and can not be used.`);
    }
    let tableName = merged.tableName || merged.name;
    if (isReserved(merged.name, 'TODO')) {
      throw new buildException(exceptions.IllegalName, `The name '${merged.name}' is reserved DB keyword and can not be used.`);
    }
