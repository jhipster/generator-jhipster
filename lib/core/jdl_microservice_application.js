...
const { getConfigForMicroserviceApplication } = require('./jhipster/default_application_options');

class JDLMicroserviceApplication extends AbstractJDLApplication {
  constructor(args) {
    const config = getConfigForMicroserviceApplication(args.config);
    super({ config, entities: args.entities });

    // TODO the three ifs are weird
