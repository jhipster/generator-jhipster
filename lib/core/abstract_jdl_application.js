...
function sanitizeConfigurationForStringifying(applicationConfiguration) {
  const optionsThatShouldNotBeExportedUnlessTheyHaveAValue = new Set(['entitySuffix', 'dtoSuffix']);
  const optionsThatShouldNotBeExported = new Set(['blueprints']);
...
function stringifyOptionValue(name, value) {
  ...
  const optionsToQuoteIfNeedBe = new Set(['jhipsterVersion', 'jwtSecretKey']);
