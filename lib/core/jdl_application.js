...
class JDLApplication {
  ...
  fixApplicationConfig(passedConfig) {
    ...
    if (!config.baseName) {
      config.baseName = ApplicationOptions.baseName;
    }
    if (!config.buildTool) {
      config.buildTool = ApplicationOptions.buildTool.maven;
    }
    if (config.enableTranslation && !config.nativeLanguage) {
      config.nativeLanguage = ApplicationOptions.nativeLanguage;
    }
    if (config.skipUserManagement !== true && config.skipUserManagement !== false) {
      config.skipUserManagement = ApplicationOptions.skipUserManagement;
    }
    return config;
  }
