...
class JDLApplication {
  ...
  static checkValidity(application) {
    ...
    if (
      application.config.authenticationType === ApplicationOptions.authenticationType.jwt &&
      !application.config.jwtSecretKey
    ) {
      errors.push(ApplicationErrorCases.NoJWTSecret);
    }
    if (
      application.config.authenticationType === ApplicationOptions.authenticationType.session &&
    ...
    if (
      (application.config.applicationType === ApplicationTypes.MICROSERVICE ||
        application.config.applicationType === ApplicationTypes.UAA) &&
      !application.config.skipClient
    ) {
      errors.push(ApplicationErrorCases.ShouldSkipClient);
    }
    if (
      !application.config.uaaBaseName &&
      (application.config.applicationType === ApplicationTypes.UAA ||
        application.config.applicationType === ApplicationTypes.MICROSERVICE) &&
...
function fixApplicationConfig(passedConfig) {
  ...
  if (passedConfig.authenticationType === ApplicationOptions.authenticationType.jwt && !passedConfig.jwtSecretKey) {
  ...
  if (
    passedConfig.authenticationType === ApplicationOptions.authenticationType.session &&
  ...
  if (
    (passedConfig.applicationType === ApplicationTypes.MICROSERVICE ||
      passedConfig.applicationType === ApplicationTypes.UAA) &&
    !passedConfig.skipClient
  ) {
    passedConfig.skipClient = true;
  }
  if (
    (passedConfig.applicationType === ApplicationTypes.GATEWAY && passedConfig.authenticationType.uaa) ||
