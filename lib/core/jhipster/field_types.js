...
const JDLSet = require('../../utils/objects/set');
...
const CommonDBValidations = {
  String: new JDLSet([REQUIRED, UNIQUE, MINLENGTH, MAXLENGTH, PATTERN]),
  Integer: new JDLSet([REQUIRED, UNIQUE, MIN, MAX]),
  Long: new JDLSet([REQUIRED, UNIQUE, MIN, MAX]),
  BigDecimal: new JDLSet([REQUIRED, UNIQUE, MIN, MAX]),
  Float: new JDLSet([REQUIRED, UNIQUE, MIN, MAX]),
  Double: new JDLSet([REQUIRED, UNIQUE, MIN, MAX]),
  Enum: new JDLSet([REQUIRED, UNIQUE]),
  Boolean: new JDLSet([REQUIRED, UNIQUE]),
  LocalDate: new JDLSet([REQUIRED], UNIQUE),
  ZonedDateTime: new JDLSet([REQUIRED, UNIQUE]),
  Blob: new JDLSet([REQUIRED, UNIQUE, MINBYTES, MAXBYTES]),
  AnyBlob: new JDLSet([REQUIRED, UNIQUE, MINBYTES, MAXBYTES]),
  ImageBlob: new JDLSet([REQUIRED, UNIQUE, MINBYTES, MAXBYTES]),
  TextBlob: new JDLSet([REQUIRED, UNIQUE]),
  Instant: new JDLSet([REQUIRED, UNIQUE])
};
...
const CassandraValidations = {
  ...
  Double: new JDLSet([REQUIRED, UNIQUE, MIN, MAX]),
  Boolean: new JDLSet([REQUIRED, UNIQUE]),
  Date: new JDLSet([REQUIRED, UNIQUE]),
  UUID: new JDLSet([REQUIRED, UNIQUE]),
  Instant: new JDLSet([REQUIRED, UNIQUE])
};
