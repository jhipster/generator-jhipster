...
class JDLRelationships {
  ...
  add(relationship) {
    if (!relationship) {
      throw new Error('A relationship must be passed.');
    }
    if (!JDLRelationship.isValid(relationship)) {
      throw new Error('A valid relationship must be passed.');
    }
    this.relationships[relationship.type][relationship.getId()] = relationship;
  }

  size() {
    return Object.keys(this.relationships.OneToOne).length + Object.keys(this.relationships.OneToMany).length
      + Object.keys(this.relationships.ManyToMany).length + Object.keys(this.relationships.ManyToMany).length;
  }
