...
class JDLRelationship {
   ...
   */
  getId() {
    return `${this.type}_${this.from}${(this.injectedFieldInFrom) ? `{${this.injectedFieldInFrom}}` : ''}`
...
function checkFromAndToTypesAreString(merged) {
  ...
  logger.warn('The \'from\' and \'to\' keys will only be accepted as strings in the next major version instead of '
...
function checkRelationshipType(relationship) {
  ...
  case RelationshipTypes.ONE_TO_ONE:
    if (!relationship.injectedFieldInFrom) {
      throw new Error(`In the One-to-One relationship from ${relationship.from} to ${relationship.to}, `
  ...
  case RelationshipTypes.MANY_TO_ONE:
    if (relationship.injectedFieldInFrom && relationship.injectedFieldInTo) {
      throw new Error(`In the Many-to-One relationship from ${relationship.from} to ${relationship.to}, `
  ...
  case RelationshipTypes.MANY_TO_MANY:
    if (!relationship.injectedFieldInFrom || !relationship.injectedFieldInTo) {
      throw new Error(`In the Many-to-Many relationship from ${relationship.from} to ${relationship.to}, `
...
function addMissingSide(relationship) {
  if (!relationship.injectedFieldInFrom) {
    relationship.injectedFieldInFrom = _.lowerFirst(relationship.to);
    return;
  }
  relationship.injectedFieldInTo = _.lowerFirst(relationship.from);
}
