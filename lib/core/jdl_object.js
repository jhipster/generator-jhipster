...
class JDLObject {
  ...
  addApplication(application) {
    ...
    if (errors.length !== 0) {
      throw new Error('The application must be valid in order to be added to the JDL object'
        + `.\nErrors: ${errors.join(', ')}`);
    }
  ...
  addRelationship(relationship) {
    ...
    if (errors.length !== 0) {
      throw new Error('The relationship must be valid in order to be added to the JDL object.'
        + `\nErrors: ${errors.join(', ')}`);
    }
