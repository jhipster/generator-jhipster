  ...
  checkForOptionErrors() {
    this.jdlObject.forEachOption(jdlOption => {
      ...
      if (
        ...
        this.applicationSettings.databaseType === CASSANDRA
      ) {
...
function checkForDTOWithoutService(jdlObject) {
  const dtoOptions = jdlObject.getOptionsForName(Options.DTO);
  if (dtoOptions.length !== 0) {
    const serviceEntities = jdlObject.getOptionsForName(Options.SERVICE).reduce((accumulated, serviceOption) => {
