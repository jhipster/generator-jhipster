...
class BusinessErrorChecker {
  ...
  checkForApplicationErrors() {
    this.jdlObject.forEachApplication((jdlApplication) => {
      ...
      const applicationIsAMicroserviceWithoutOauth2 = jdlApplication.config.authenticationType !== 'oauth2'
        && jdlApplication.config.applicationType === ApplicationTypes.MICROSERVICE;
      const applicationAGatewayWithUaa = jdlApplication.config.authenticationType === 'uaa'
        && jdlApplication.config.applicationType === ApplicationTypes.GATEWAY;
      if (jdlApplication.config.databaseType === DatabaseTypes.NO && !(applicationIsAMicroserviceWithoutOauth2 || applicationAGatewayWithUaa)) {
        throw new Error('Having no database type is only allowed for '
          + 'microservices without oauth2 authentication type and gateways with UAA authentication type.');
      }
    });
  }
  ...
  checkForRelationshipErrors() {
    this.jdlObject.forEachRelationship((jdlRelationship) => {
      ...
      if (jdlRelationship.from.name.toLowerCase() === jdlRelationship.to.name.toLowerCase()
          && (jdlRelationship.isInjectedFieldInFromRequired || jdlRelationship.isInjectedFieldInToRequired)) {
        throw new Error('Required relationships to the same entity are not supported, for relationship from '
          + `'${jdlRelationship.from.name}' to '${jdlRelationship.to.name}'.`);
      }
      if (!this.jdlObject.getEntity(jdlRelationship.from.name)) {
        absentEntities.push(jdlRelationship.from.name);
      }
      if (jdlRelationship.to.name.toLowerCase() !== USER.toLowerCase() && !this.jdlObject.getEntity(jdlRelationship.to.name)) {
  ...
  applications.forEach((application) => {
    if (!typeCheckingFunction) { // first iteration
      ...
      if (newIsType !== typeCheckingFunction) {
        logger.warn(`Multiple applications are declared to have entity '${entityName}', check field type to ensure`
          + ' compatibility.');
      }
...
function checkForDTOWithoutService(jdlObject) {
  ...
  if (dtoOptions.length !== 0) {
    const serviceEntities = jdlObject
      ...
      .reduce((accumulated, serviceOption) => accumulated.concat(serviceOption.entityNames.toArray()), []);
    const dtoEntities = dtoOptions.reduce((accumulated, dtoOption) => accumulated.concat(dtoOption.entityNames.toArray()), []);
