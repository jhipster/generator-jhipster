  ...
  checkForValidationErrors(jdlField, isAnEnum) {
    Object.keys(jdlField.validations).forEach((validationName) => {
      ...
      if (!FieldTypes.hasValidation(jdlField.type, jdlValidation.key, isAnEnum)) {
        throw new Error(`The validation '${jdlValidation.key}' isn't supported for the type '${jdlField.type}'.`);
      }
    });
  }
  ...
  checkForEnumErrors() {
    Object.keys(this.jdlObject.enums).forEach((jdlEnum) => {
      if (isReservedClassName(jdlEnum.name)) {
        throw new Error(`The name '${jdlEnum.name}' is reserved keyword and can not be used as enum class name.`);
      }
    });
  }
  ...
  checkForOptionErrors() {
    this.jdlObject.options.getOptions().forEach((jdlOption) => {
      ...
      if (!!jdlOption.value && !JDLBinaryOption.isValid(jdlOption)) {
        throw new Error(`The parsed ${jdlOption.name} option is not valid for value ${jdlOption.value}.`);
      }
    });
  }
}
