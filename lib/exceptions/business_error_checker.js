...
const BuildException = require('./exception_factory').BuildException;
const exceptions = require('./exception_factory').exceptions;
...
class BusinessErrorChecker {
  ...
  checkForEntityErrors() {
    ...
    Object.keys(this.jdlObject.entities).forEach((entityName) => {
      ...
      this.checkForFieldErrors(jdlEntity.fields);
    });
  }
  ...
  checkForFieldErrors(entityName, jdlFields) {
    Object.keys(jdlFields).forEach((fieldName) => {
      ...
      if (isReservedFieldName(jdlField.name)) {
        throw new BuildException(
          exceptions.IllegalName,
          `The name '${jdlField.name}' is a reserved keyword and can not be used as entity field name.`);
      }
      const typeCheckingFunction = getTypeCheckingFunction(entityName, this.applicationsPerEntityName);
      if (!this.jdlObject.enums[jdlField.type] && typeCheckingFunction(jdlField.type)) {
        throw new BuildException(
          exceptions.IllegalName,
          `The name '${fieldName}' is a reserved keyword and can not be used as entity field name.`);
      }
      this.checkForValidationErrors(jdlField, this.jdlObject.enums[jdlField.type]);
    });
  }
  ...
  checkForValidationErrors(jdlField, isAnEnum) {
    Object.keys(jdlField.validations).forEach((validationName) => {
      ...
      if (!FieldTypes.hasValidation(jdlField.type, jdlValidation.key, isAnEnum)) {
        throw new BuildException(
          exceptions.WrongValidation,
          `The validation '${jdlValidation.key}' isn't supported for the type '${jdlField.type}'.`);
      }
    });
  }
    ...
    this.jdlObject.relationships.toArray().forEach((jdlRelationship) => {
      ...
      if (jdlRelationship.from.name.toLowerCase() === USER.toLowerCase()) {
        throw new BuildException(
          ...
          `Relationships from User entity is not supported in the declaration between ${jdlRelationship.from.name} and `
  ...
  checkForEnumErrors() {
    Object.keys(this.jdlObject.enums).forEach((jdlEnum) => {
      if (isReservedClassName(jdlEnum.name)) {
        throw new BuildException(
          exceptions.IllegalName,
          `The name '${jdlEnum.name}' is reserved keyword and can not be used as enum class name.`);
      }
    });
  }
  ...
  checkForOptionErrors() {
    this.jdlObject.options.getOptions().forEach((jdlOption) => {
      if (jdlOption.getType() === 'BINARY' && jdlOption.name === BinaryOptions.Options.PAGINATION
      ...
      if (!!jdlOption.value && !JDLBinaryOption.isValid(jdlOption)) {
        throw new BuildException(
          ...
          `The parsed ${jdlOption.name} option is not valid for value ${jdlOption.value}.`);
      }
    });
  }
}
...
function getTypeCheckingFunction(entityName, applicationsPerEntityName) {
  ...
  applications.forEach((application) => {
    if (!typeCheckingFunction) { // first iteration
      typeCheckingFunction = FieldTypes.getIsType(application.config.databaseType, () => { typeCheckingFunction = null; });
    } else {
      const newIsType = FieldTypes.getIsType(application.config.databaseType, () => { typeCheckingFunction = null; });
