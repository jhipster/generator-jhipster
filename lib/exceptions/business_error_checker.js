...
const RelationshipOptions = require('../core/jhipster/relationship_options');
...
class BusinessErrorChecker {
  ...
  checkForRelationshipErrors() {
    this.jdlObject.forEachRelationship(jdlRelationship => {
      ...
      if (
        jdlRelationship.type !== RelationshipTypes.ONE_TO_ONE &&
        jdlRelationship.hasOption(RelationshipOptions.JPA_DERIVED_IDENTIFIER)
      ) {
        throw new Error(
          `Only a One to One relationship can have the '${RelationshipOptions.JPA_DERIVED_IDENTIFIER}' option.`
        );
      }
...
function checkRelationshipOptions(jdlRelationship) {
  ...
  jdlRelationship.forEachOption(option => {
    if (!RelationshipOptions.exists(option)) {
      invalidOptions.push(option);
    }
  });
  if (invalidOptions.length !== 0) {
    throw new Error(
      `These relationship options do not exist: '${invalidOptions.join(', ')}' for relationship from '${
        jdlRelationship.from
      }' to '${jdlRelationship.to}'.`
    );
  }
}
