...
function checkForInvalidDatabaseCombinations() {
  const { databaseType, devDatabaseType, prodDatabaseType } = applicationConfig;

  if (databaseType === NO && (devDatabaseType !== NO || prodDatabaseType !== NO)) {
    throw new Error(
      "Having 'no' as databaseType requires that devDatabaseTypes and prodDatabaseTypes values are also 'no'."
    );
  }
  if (databaseType === SQL) {
...
function stripCustomOptions() {
  const configCopy = { ...applicationConfig };
...
function checkForNoDatabaseType() {
  const { authenticationType, applicationType, databaseType } = applicationConfig;

  const applicationIsAMicroserviceWithoutOauth2 = authenticationType !== oauth2 && applicationType === MICROSERVICE;
  const applicationAGatewayWithUaa = authenticationType === uaa && applicationType === GATEWAY;
  if (databaseType === NO && !(applicationIsAMicroserviceWithoutOauth2 || applicationAGatewayWithUaa)) {
    throw new Error(
