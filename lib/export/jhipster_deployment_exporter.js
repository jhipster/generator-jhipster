...
const { toArray } = require('../utils/set_utils');
...
function setUpArrayOptions(deployment) {
  deployment[GENERATOR_NAME].appsFolders = toArray(deployment[GENERATOR_NAME].appsFolders);
  deployment[GENERATOR_NAME].consoleOptions = toArray(deployment[GENERATOR_NAME].consoleOptions);
  deployment[GENERATOR_NAME].clusteredDbApps = toArray(deployment[GENERATOR_NAME].clusteredDbApps);
  return deployment;
}
...
function writeDeploymentConfigs(deployment) {
  ...
  delete deployment[GENERATOR_NAME].deploymentType;
