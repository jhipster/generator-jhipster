...
module.exports = class SharedData {
  constructor(configOptions) {
    ...
    this._configOptions.sharedEntities = this._configOptions.sharedEntities || {};
    this._configOptions.sharedApplication = this._configOptions.sharedApplication || {};
  }
  ...
  getApplication() {
    if (!this._configOptions.sharedApplication) throw new Error('Shared application not loaded');
    return this._configOptions.sharedApplication;
  }

  setEntity(entityName, entity) {
    this._configOptions.sharedEntities[entityName] = entity;
  }

  hasEntity(entityName) {
    return Boolean(this._configOptions.sharedEntities[entityName]);
  }

  getEntity(entityName) {
    const entity = this._configOptions.sharedEntities[entityName];
    if (!entity) {
  ...
  getEntities(entityNames = Object.keys(this._configOptions.sharedEntities)) {
