...
const { PRIORITY_PREFIX } = require('./priorities.cjs');
...
module.exports.generateMixedChain = (...generators) => {
  ...
  let MixedClass = class extends GeneratorBaseBlueprint {
  ...
  MixedClass.prototype.prepareDerivedChainProperties = function (...args) {
    executeChain(
      this,
      args,
      upperCamelCaseGenerators.map(upperCamelCaseGenerator => `prepareDerived${upperCamelCaseGenerator}Properties`)
    );
  };
