 ...
 */
const convertToPrettierExpressions = str => {
 ...
 * @returns
 */
const checkContentIn = (contentToCheck, content, ignoreWhitespaces = true) => {
  ...
  if (contentToCheck.test) {
    ...
    re = convertToPrettierExpressions(escapeRegExp(contentToCheck));
  } else {
    re = contentToCheck
      .split('\n')
 ...
 */
const insertContentBeforeNeedle = ({ content, contentToAdd, needle }) => {
 ...
 */
const createNeedleCallback = ({ needle, contentToAdd, optional = false, ignoreWhitespaces = true }) => {
  assert(needle, 'needle is required');
  assert(contentToAdd, 'contentToAdd is required');

  return function (content, filePath) {
    if (checkContentIn(contentToAdd, content, ignoreWhitespaces)) {
      return content;
    }
...
const createBaseNeedle = (options, needles) => {
  ...
  const callbacks = Object.entries(needles)
    ...
    .map(([key, contentToAdd]) => createNeedleCallback({ needle: kebabCase(key), contentToAdd, optional, ignoreWhitespaces }));
