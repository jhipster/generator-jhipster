...
function prepareEntityForTemplates(entityWithConfig, generator) {
    ...
    if (entityWithConfig.currentEntityState && entityWithConfig.currentEntityState.fields && entityWithConfig.currentEntityState.fields.length > 0) {
        ...
        if (
            (entityWithConfig.newFields && entityWithConfig.newFields.length > 0) ||
            (entityWithConfig.removedFields && entityWithConfig.removedFields.length > 0)
        ) {
    ...
    if (
        entityWithConfig.currentEntityState &&
        entityWithConfig.currentEntityState.relationships &&
        entityWithConfig.currentEntityState.relationships.length > 0
    ) {
        entityWithConfig.newRelationships = getNewElements(entityWithConfig.relationships, entityWithConfig.currentEntityState.relationships);
        entityWithConfig.removedRelationships = getRemovedElements(entityWithConfig.relationships, entityWithConfig.currentEntityState.relationships);

        if (
            (entityWithConfig.newRelationships && entityWithConfig.newRelationships.length > 0) ||
            (entityWithConfig.removedRelationships && entityWithConfig.removedRelationships.length > 0)
        ) {
            entityWithConfig.newChangelog = true;
        }
    }
