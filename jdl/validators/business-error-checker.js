...
const { GATEWAY } = require('../jhipster/application-types');
const FieldTypes = require('../jhipster/field-types');
const { Options } = require('../jhipster/binary-options');
const { CASSANDRA } = require('../jhipster/database-types');
const { isReservedFieldName, isReservedTableName } = require('../jhipster/reserved-keywords');
...
class BusinessErrorChecker {
    ...
    checkForEntityErrors() {
        ...
        this.jdlObject.forEachEntity(jdlEntity => {
            ...
            if (this.applicationSettings.databaseType && isReservedTableName(jdlEntity.tableName, this.applicationSettings.databaseType)) {
                logger.warn(
...
function getTypeCheckingFunction(entityName, applicationSettings, applicationsPerEntityName) {
    ...
    if (!applications || (applications.length === 1 && applications[0].getConfigurationOptionValue('databaseType') === GATEWAY)) {
        return () => true;
    }
