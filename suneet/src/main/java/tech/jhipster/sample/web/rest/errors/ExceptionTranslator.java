...
public class ExceptionTranslator extends ResponseEntityExceptionHandler {
    ...
    private ProblemDetailWithCause getProblemDetailWithCause(Throwable ex) {
        if (ex instanceof tech.jhipster.sample.service.EmailAlreadyUsedException) return new EmailAlreadyUsedException()
            .getProblemDetailWithCause();
        if (ex instanceof tech.jhipster.sample.service.UsernameAlreadyUsedException) return new LoginAlreadyUsedException()
            .getProblemDetailWithCause();
        if (
            ex instanceof tech.jhipster.sample.service.InvalidPasswordException
    ...
    protected ProblemDetailWithCause customizeProblem(ProblemDetailWithCause problem, Throwable err, NativeWebRequest request) {
        ...
        if (problem.getProperties() == null || !problem.getProperties().containsKey(MESSAGE_KEY)) problem.setProperty(
            MESSAGE_KEY,
            getMappedMessageKey((Throwable) err) != null ? getMappedMessageKey(err) : "error.http." + problem.getStatus()
        );

        if (problem.getProperties() == null || !problem.getProperties().containsKey(PATH_KEY)) problem.setProperty(
            PATH_KEY,
            getPathValue(request)
        );
