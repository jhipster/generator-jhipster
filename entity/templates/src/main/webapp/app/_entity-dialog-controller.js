(function () {
    ...
    controller.$inject = ['$stateParams'<% if (fieldsContainOwnerOneToOne) { %>,
        '$q'<% } %>,
        'entity',
    ...
    function controller($stateParams<% if (fieldsContainOwnerOneToOne) { %>,
            $q<% } %>,
            entity,
            <%= entityClass %><% for (idx in differentTypes) { if (differentTypes[idx] != entityClass) {%>,
            <%= differentTypes[idx] %><% } } %>){
            ...
            var onSaveFinished = function (result) {
                vm.$emit('<%=angularAppName%>:<%= entityInstance %>Update', result);
