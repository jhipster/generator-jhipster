...
public class <%= entityClass %>Resource {
    ...
    @RequestMapping(value = "/rest/<%= entityInstance %>s/{id}",
            ...
            produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<<%= entityClass %>> get(@PathVariable <%= primaryKeyAsObjectType %> id<% if (javaVersion == '7') { %>, HttpServletResponse response<% } %>) {
    ...
    @RequestMapping(value = "/rest/<%= entityInstance %>s/{id}",
            ...
            produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public void delete(@PathVariable <%= primaryKeyAsObjectType %> id) {
