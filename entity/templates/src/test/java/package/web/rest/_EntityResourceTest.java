...
public class <%= entityClass %>ResourceTest {
    <% if (numberPrimaryKey) { %>
    private static final Long DEFAULT_ID = new Long(1L);<% } %><% if (stringPrimaryKey) { %>
    ...
    public void testCRUD<%= entityClass %>() throws Exception {
    	...
    	rest<%= entityClass %>MockMvc.perform(get("/app/rest/<%= entityInstance %>s/{id}", DEFAULT_ID))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))<% if (numberPrimaryKey) { %>
                .andExpect(jsonPath("$.id").value(DEFAULT_ID.intValue()))<% } %><% if (stringPrimaryKey) { %>
