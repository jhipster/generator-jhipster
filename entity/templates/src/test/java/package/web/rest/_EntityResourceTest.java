...
<% for (fieldId in fields) {
    ...
    public void get<%= entityClass %>() throws Exception {
        ...
        rest<%= entityClass %>MockMvc.perform(get("/api/<%= entityInstance %>s/{<%= primaryKeyField.fieldName %>}", <%= entityInstance %>.get<%= primaryKeyField.fieldNameCapitalized %>()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON))<% if (databaseType == 'sql' && pkManagedByJHipster == true) { %>
    ...
    public void getNonExisting<%= entityClass %>() throws Exception {
        // Get the <%= entityInstance %>
        rest<%= entityClass %>MockMvc.perform(get("/api/<%= entityInstance %>s/{<%= primaryKeyField.fieldName %>}", 1L))
    ...
    public void update<%= entityClass %>() throws Exception {
        ...
        List<<%= entityClass %>> <%= entityInstance %>s = <%= entityInstance %>Repository.findAll();
        assertThat(<%= entityInstance %>s).hasSize(1);
        <%= entityClass %> test<%= entityClass %> = <%= entityInstance %>s.iterator().next();<% for (fieldId in fields) {%><% if (fields[fieldId].isPk == false) { if (fields[fieldId].fieldType == 'DateTime') { %>
        assertThat(test<%= entityClass %>.get<%=fields[fieldId].fieldNameCapitalized%>()).isEqualTo(<%='UPDATED_' + fields[fieldId].fieldNameUnderscored.toUpperCase()%>);<% } %>;<% } else { %>
    ...
    public void delete<%= entityClass %>() throws Exception {
        ...
        // Get the <%= entityInstance %>
        rest<%= entityClass %>MockMvc.perform(delete("/api/<%= entityInstance %>s/{<%= primaryKeyField.fieldName %>}", <%= entityInstance %>.get<%= primaryKeyField.fieldNameCapitalized %>())
