...
<% if (authenticationType == 'session') { %>
    ...
    val scn = scenario("Test the <%= entityClass %> entity")
        ...
        .repeat(2) {
            ...
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null<% for (fieldId in fields) { %>, "<%= fields[fieldId].fieldName %>":<% if (fields[fieldId].fieldType == 'String') { %>"SAMPLE_TEXT"<% } else if (fields[fieldId].fieldType == 'Integer') { %>"0"<% } else if (fields[fieldId].fieldType == 'DateTime' || fields[fieldId].fieldType == 'ZonedDateTime' || fields[fieldId].fieldType == 'LocalDate') { %>"2020-01-01T00:00:00.000Z"<% } else { %>null<% } } %>}""")).asJSON
            .check(status.is(201))
