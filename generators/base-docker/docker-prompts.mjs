...
async function askForServiceDiscovery() {
  ...
  if (serviceDiscoveryEnabledApps.every(app => app.serviceDiscoveryType === CONSUL)) {
    ...
    this.logguer.info(chalk.green('Consul detected as the service discovery and configuration provider used by your apps'));
  } else if (serviceDiscoveryEnabledApps.every(app => app.serviceDiscoveryType === EUREKA)) {
    this.serviceDiscoveryType = EUREKA;
    this.logguer.info(chalk.green('JHipster registry detected as the service discovery and configuration provider used by your apps'));
  } else {
    this.logguer.warn(
      chalk.yellow('Unable to determine the service discovery and configuration provider to use from your apps configuration.')
    );
    this.logguer.info('Your service discovery enabled apps:');
    serviceDiscoveryEnabledApps.forEach(app => {
      this.logguer.info(` -${app.baseName} (${app.serviceDiscoveryType})`);
    });
