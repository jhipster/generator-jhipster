...
export default class HerokuGenerator extends BaseGenerator {
  ...
  get initializing() {
    return {
      ...
      initializing() {
        this.logguer.info(chalk.bold('Heroku configuration is starting'));
      ...
      askForApp() {
        ...
        if (this.herokuAppName) {
          ChildProcess.exec(`heroku apps:info --json ${this.herokuAppName}`, (err, stdout) => {
            if (err) {
              this.abort = true;
              this.logguer.error(`Could not find application: ${chalk.cyan(this.herokuAppName)}`);
              this.logguer.error('Run the generator again to create a new application.');
              ...
              this.logguer.info(`Deploying as existing application: ${chalk.bold(this.herokuAppName)}`);
    ...
    return {
      checkInstallation() {
        ...
        ChildProcess.exec('heroku --version', err => {
          if (err) {
            this.logguer.error("You don't have the Heroku CLI installed. Download it from https://cli.heroku.com/");
            this.abort = true;
          }
          done();
        });
      ...
      gitInit() {
        ...
        try {
          ...
          child.stdout.on('data', data => {
            this.logguer.info(data.toString());
          });
        }
      },
          ...
          if (err) {
            if (stderr.includes('is already taken')) {
              ...
              if (stderr.includes('Invalid credentials')) {
                this.logguer.error("Error: Not authenticated. Run 'heroku login' to login to your heroku account and try again.");
              } else {
                this.logguer.error(err);
              }
              done();
        ...
        const addonCreateCallback = (addon, err, stdout, stderr) => {
          if (err) {
            ...
            if (_.includes(err, verifyAccountUrl)) {
              this.abort = true;
              this.logguer.error(`Account must be verified to use addons. Please go to: ${verifyAccountUrl}`);
              this.logguer.error(err);
            } else {
              this.logguer.info(`No new ${addon} addon created`);
            }
          } else {
            this.logguer.info(`Created ${addon} addon`);
          }
        };
      ...
      copyHerokuFiles() {
        ...
        if (this.useOkta) {
          ...
          fs.appendFile('.gitignore', 'provision-okta-addon.sh', 'utf8', (err, data) => {
            if (err) {
              this.logguer.warn(`${chalk.yellow.bold('WARNING!')} Failed to add 'provision-okta-addon.sh' to .gitignore.'`);
            }
          });
        }
      },
      ...
      async productionDeploy() {
        ...
        if (this.herokuDeployType === 'git') {
          try {
            ...
            this.logguer.warn(chalk.yellow(`And you can view the logs with this command\n\t${chalk.bold('heroku logs --tail')}`));
            this.logguer.warn(chalk.yellow(`After application modification, redeploy it with\n\t${chalk.bold('jhipster heroku')}`));
            ...
            if (this.useOkta) {
              ...
              try {
                ...
                this.logguer.warn(chalk.yellow('After you have installed curl execute ./provision-okta-addon.sh manually.'));
              }
              try {
                ...
                this.logguer.warn(chalk.yellow('After you have installed jq execute ./provision-okta-addon.sh manually.'));
              }
              if (curlAvailable && jqAvailable) {
                this.logguer.info(chalk.green('Running ./provision-okta-addon.sh to create all required roles and users for JHipster.'));
                try {
