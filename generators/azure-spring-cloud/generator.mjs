...
export default class AzureSpringCloudGenerator extends BaseGenerator {
  ...
  get initializing() {
    return {
      sayHello() {
        this.logguer.info(chalk.bold('Azure Spring Cloud configuration is starting'));
      },
      getSharedConfig() {
        this.loadAppConfig();
    ...
    return {
      checkBuildTool() {
        ...
        if (this.buildTool !== MAVEN) {
          this.logguer.error('Sorry, this sub-generator only works with Maven projects for the moment.');
          this.abort = true;
        }
        done();
      },
...
${chalk.red('az extension add --name spring-cloud')}`
      ...
      checkClusterAvailability() {
        ...
        exec('az spring-cloud app list', err => {
          if (err) {
            this.logguer.error(`${chalk.red('Your Azure Spring Cloud cluster is not available!')}\n ${err}`);
            this.abort = true;
          }
          done();
        });
      ...
      getAzureSpringCloudDefaults() {
        ...
        exec('az configure --list-defaults true', (err, stdout) => {
          if (err) {
            ...
            this.abort = true;
            this.logguer.error('Could not retrieve your Azure default configuration.');
          } else {
...
--service ${this.azureSpringCloudServiceName} --name ${this.azureSpringCloudAppName}`,
          (err, stdout) => {
            if (err) {
              this.logguer.info(chalk.bold('Application does not exist yet, creating it...'));
              exec(
            ...
            --service ${this.azureSpringCloudServiceName} --name ${this.azureSpringCloudAppName}`,
                (err, stdout) => {
                  if (err) {
                    this.abort = true;
                    this.logguer.error(`Application creation failed! Here is the error: ${err}`);
                  } else {
                    this.logguer.info(`${chalk.green(chalk.bold('Success!'))} Your application has been created.`);
                  }
                  done();
                }
              );
            } else {
              this.logguer.info(chalk.bold('Application already exists, using it.'));
              done();
            }
          }
        );
...
--jar-path ${buildDir}/*.jar`,
          (err, stdout) => {
            if (err) {
              this.abort = true;
              this.logguer.error(`Deployment failed!\n ${err}`);
            } else {
              const json = JSON.parse(stdout);
              this.logguer.info(`${chalk.green(chalk.bold('Success!'))} Your application has been deployed.`);
              this.logguer.info(`Provisioning state: ${chalk.bold(json.properties.provisioningState)}`);
              this.logguer.info(`Application status  : ${chalk.bold(json.properties.status)}`);
            }
            done();
          }
        );
