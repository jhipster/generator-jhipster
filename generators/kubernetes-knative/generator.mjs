...
export default class KubernetesKnativeGenerator extends BaseDockerGenerator {
  ...
  get initializing() {
    return {
      sayHello() {
        this.logguer.info(chalk.white(`${chalk.bold('☸')} Welcome to the JHipster Kubernetes Knative Generator ${chalk.bold('☸')}`));
        this.logguer.info(chalk.white(`Files will be generated in the folder: ${chalk.yellow(this.destinationRoot())}`));
      },
      ...super.initializing,
      checkKubernetes,
      checkHelm,
    ...
    return {
      deploy() {
        ...
        if (this.generatorType === K8S) {
          this.logguer.info(`  ${chalk.cyan('bash kubectl-knative-apply.sh')}`);
          ...
          this.logguer.info(`  ${chalk.cyan('bash helm-knative-apply.sh or ./helm-knative-apply.sh')}`);
          this.logguer.info('\nYou can upgrade (after any changes) all your apps by running the following script:');
          this.logguer.info(`  ${chalk.cyan('bash helm-knative-upgrade.sh or ./helm-knative-upgrade.sh')}`);
          // Make the apply script executable
          try {
