...
const packageInfoTransform = ({ javaRoots, javadocs }: PackageInfoTransformOptions) => {
  const existingPackageInfoFiles: Set<string> = new Set();
  const packageInfoFilesToWrite: Record<string, { root: string }> = {};
  return passthrough(
    function (file) {
      if (file.path.endsWith('package-info.java')) {
        existingPackageInfoFiles.add(file.path);
        return;
      }
      for (const root of javaRoots) {
        if (file.path.startsWith(root)) {
          ...
          packageInfoFilesToWrite[packageInfoFilePath] = { root };
        }
      }
    },
    function () {
      for (const [packageInfoFilePath, { root }] of Object.entries(packageInfoFilesToWrite)) {
        if (existingPackageInfoFiles.has(packageInfoFilePath)) {
          return;
        }
        const directory = dirname(packageInfoFilePath);
...
`);
        ...
        existingPackageInfoFiles.add(packageInfoFilePath);
      }
    },
  );
};
