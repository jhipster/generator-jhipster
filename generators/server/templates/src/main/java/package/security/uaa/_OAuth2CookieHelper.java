...
public class OAuth2CookieHelper {
    ...
    public static Cookie getClientAuthorizationCookie(HttpServletRequest request) {
        return getCookie(request, CLIENT_AUTHORIZATION_COOKIE);
    }
     ...
     * @param refreshTokenCookie the refresh token cookie we already created.
     * @return the cookie to set on the response.
     */
    public Cookie createClientAuthorizationCookie(HttpServletRequest request,
                                                  Cookie refreshTokenCookie) {
        String authorization = request.getHeader("Authorization");
        if (authorization != null) {
            Cookie clientAuthorizationCookie =
                new Cookie(CLIENT_AUTHORIZATION_COOKIE, authorization);
            setCookieProperties(clientAuthorizationCookie, request, refreshTokenCookie.getDomain());
            clientAuthorizationCookie.setMaxAge(refreshTokenCookie.getMaxAge());
    ...
    public void createCookies(HttpServletRequest request, OAuth2AccessToken accessToken, boolean rememberMe,
        ...
        setCookieProperties(accessTokenCookie, request, domain);
        ...
        if (rememberMe) {           //keep token around if rememberMe is true
            ...
            refreshTokenCookieName = PERSISTENT_REFRESH_TOKEN_COOKIE;
        }
        else {
            refreshTokenCookieName = SESSION_REFRESH_TOKEN_COOKIE;
        }
        Cookie refreshTokenCookie = new Cookie(refreshTokenCookieName, refreshToken.getValue());
        refreshTokenCookie.setMaxAge(maxAge);
        log.debug("created refresh token cookie '{}', age: {}", refreshTokenCookie.getName(), maxAge);
        Cookie clientAuthorizationCookie = createClientAuthorizationCookie(request, refreshTokenCookie);
    ...
    private void clearCookie(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse,
        ...
        setCookieProperties(cookie, httpServletRequest, domain);
        cookie.setMaxAge(0);
