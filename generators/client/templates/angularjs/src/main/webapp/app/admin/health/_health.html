    ...
    <table id="healthCheck" class="table table-striped">
        ...
        <tbody>
            <tr ng-repeat="health in vm.healthData">
                <td><% if (enableTranslation){ %>{{'health.indicator.' + vm.baseName(health.name) | translate}}<% } else { %><span class="text-capitalize">{{ vm.baseName(health.name) }}</span><% } %> {{vm.subSystemName(health.name)}}</td>
