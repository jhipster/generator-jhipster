  ...
  <div>
    <h2>
      ...
      <button class="btn btn-primary float-right" v-on:click="refresh()">
        <font-awesome-icon icon="sync"></font-awesome-icon> <span
          v-text="$t('metrics[\'refresh.button\']')">Refresh</span>
      </button>
    </h2>
    ...
    <div class="row" v-if="!updatingMetrics">
        ...
        <div>
          <div v-for="(entry, key) of metrics.jvm" :key="key">
            ...
            <b-progress v-if="entry.max !== -1" variant="success" animated :max="entry.max" striped>
              <b-progress-bar :value="entry.used" :label="formatNumber1((entry.used * 100) / entry.max) + '%'">
              </b-progress-bar>
            </b-progress>
          </div>
        </div>
      ...
      <div class="col-md-4">
        ...
        <span><span v-text="$t('metrics.jvm.threads.runnable')">Runnable</span>
          {{ threadStats.threadDumpRunnable }}</span>
        ...
        <span><span v-text="$t('metrics.jvm.threads.timedwaiting')">Timed Waiting</span>
          ({{ threadStats.threadDumpTimedWaiting }})</span>
        <b-progress variant="success" :max="threadStats.threadDumpAll" striped>
        ...
        <span><span v-text="$t('metrics.jvm.threads.waiting')">Waiting</span>
          ({{ threadStats.threadDumpWaiting }})</span>
        <b-progress variant="success" :max="threadStats.threadDumpAll" striped>
          <b-progress-bar :value="threadStats.threadDumpWaiting"
            :label="formatNumber1((threadStats.threadDumpWaiting * 100) / threadStats.threadDumpAll) + '%'">
          </b-progress-bar>
        </b-progress>

        <span><span v-text="$t('metrics.jvm.threads.blocked')">Blocked</span>
          ({{ threadStats.threadDumpBlocked }})</span>
        <b-progress variant="success" :max="threadStats.threadDumpAll" striped>
          <b-progress-bar :value="threadStats.threadDumpBlocked"
      ...
      <div class="col-md-4">
        <div>
          ...
          <b-progress variant="success" :max="metrics.garbageCollector['jvm.gc.max.data.size']" striped>
            <b-progress-bar :value="metrics.garbageCollector['jvm.gc.live.data.size']" :label="
                formatNumber2(
      ...
      <div class="col-md-4">
        <div>
          ...
          <b-progress variant="success" :max="metrics.garbageCollector['jvm.gc.memory.allocated']" striped>
            <b-progress-bar :value="metrics.garbageCollector['jvm.gc.memory.promoted']" :label="
                formatNumber2(
    ...
    <table class="table table-striped" v-if="!updatingMetrics && isObjectExisting(metrics, 'http.server.requests')" aria-describedby="Jvm http">
      <thead>
        <tr>
    ...
    <div class="table-responsive" v-if="!updatingMetrics && isObjectExistingAndNotEmpty(metrics, 'databases')">
      <table class="table table-striped" aria-describedby="Connection pool">
        <thead>
          <tr>
            <th scope="col">
              <span v-text="$t('metrics.datasource.usage')">Connection Pool Usage</span> (active:
              {{ metrics.databases.active.value }}, min:
    ...
    <b-modal ref="metricsModal" size="lg">
      <h4 slot="modal-title" class="modal-title" id="showMetricsLabel" v-text="$t('metrics.jvm.threads.dump.title')">
        Threads dump</h4>
