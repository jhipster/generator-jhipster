...
export default class DockerComposeGenerator extends BaseDockerGenerator {
  ...
  get configuring() {
    return {
      sayHello() {
        this.logguer.info(chalk.white(`${chalk.bold('🐳')}  Welcome to the JHipster Docker Compose Sub-Generator ${chalk.bold('🐳')}`));
  ...
  get end() {
    return {
        ...
        if (this.hasWarning) {
          ...
          this.logguer.warn(chalk.red(this.warningMessage));
        } else {
          this.logguer.info(`\n${chalk.bold.green('Docker Compose configuration successfully generated!')}`);
        }
        this.logguer.info(`You can launch all your infrastructure by running : ${chalk.cyan('docker compose up -d')}`);
        if (this.gatewayNb + this.monolithicNb > 1) {
          this.logguer.info('\nYour applications will be accessible on these URLs:');
          this.appConfigs.forEach(appConfig => {
            if (appConfig.applicationType === GATEWAY || appConfig.applicationType === MONOLITH) {
              this.logguer.info(`\t- ${appConfig.baseName}: http://localhost:${appConfig.composePort}`);
            }
          });
          this.logguer.log('\n');
        }
