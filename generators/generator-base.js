...
Generator.prototype.copyTemplate = function (source, dest, action, generator, opt, template) {
    ...
    switch (action) {
    case 'stripHtml' :
        regex = /( translate-data\="([a-zA-Z0-9\ \+\{\}\'](\.)?)+")|( translate-values\="\{([a-zA-Z]|\d|\:|\{|\}|\[|\]|\-|\'|\s|\.)*?\}")|( translate-compile)|( translate-value-max\="[0-9\{\}\(\)\|]*")/g;
        //looks for something like translate-data="foo.bar.message" and translate-values="{foo: '{{ foo.bar }}'}"
