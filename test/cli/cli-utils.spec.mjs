...
import { toString, getOptionAsArgs, getCommand } from '../../cli/utils.mjs';
...
describe('jhipster cli utils test', () => {
  describe('toString', () => {
    ...
    describe('should convert array to string', () => {
      it('returns a string', () => {
        expect(toString(['test', 'foo'])).to.equal('test, foo');
        expect(toString([10, true, 'test'])).to.equal('10, true, test');
      });
    });
    describe('should convert simple objects to string', () => {
      it('returns a string', () => {
        expect(toString({ string: 'test', bool: true, int: 10 })).to.equal('string: test, bool: true, int: 10');
      });
    });
    describe('should convert complex objects to string', () => {
      it('returns a string', () => {
        expect(toString({ string: 'test', bool: true, int: 10, array: [1, 2], obj: { test: 1 } })).to.equal(
