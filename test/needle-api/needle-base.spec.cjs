...
const NeedleApiBase = require('../../generators/needle-base.cjs');
...
describe('Unit tests for needle-base API', () => {
  ...
  describe('generate a file model with a path', () => {
    before(done => {
      this.generatedModel = this.needleApiBase.generateFileModelWithPath('aPath', 'dummyFile', 'a-needle-tag', '<p>My content added</p>');
      done();
    });
