...
describe('JHipster Utils', () => {
    ...
    describe('::parseLiquibaseChangelogDate', () => {
        describe('when not passing parameters', () => {
            it('throws', () => {
                expect(() => utils.parseLiquibaseChangelogDate()).to.throw(/^A valid changelogDate is required to parse\.$/);
            });
        });
        describe('when passing an invalid changelogDate', () => {
            it('throws', () => {
                expect(() => utils.parseLiquibaseChangelogDate('1234')).to.throw(/^A valid changelogDate is required to parse\.$/);
            });
        });
        describe('when passing a valid changelogDate', () => {
            it('returns a date object', () => {
                assert(utils.parseLiquibaseChangelogDate('20160208210114') instanceof Date);
            });
            it('returns the date', () => {
                assert.equal(utils.parseLiquibaseChangelogDate('20160208210114').toISOString(), '2016-02-08T21:01:14.000Z');
            });
        });
    });
    describe('::getRecentDateForLiquibase', () => {
        ...
        describe('when not passing parameters', () => {
            it('returns a date object', () => {
                assert(utils.getRecentDateForLiquibase() instanceof Date);
            });
        });
        describe('when passing 1 day', () => {
            it('returns a date object', () => {
                assert(utils.getRecentDateForLiquibase(1) instanceof Date);
            });
        });
        describe('when passing 1 day with changelogDate', () => {
            it('returns a date object', () => {
                assert(utils.getRecentDateForLiquibase(1, '20160208210114') instanceof Date);
            });
            it('returns a recent reproducible date', () => {
                assert.equal(utils.getRecentDateForLiquibase(1, '20160208210114').toISOString(), '2016-02-08T18:17:47.710Z');
            });
        });
        describe('when passing 10 day with changelogDate', () => {
            it('returns a recent reproducible date', () => {
                assert.equal(utils.getRecentDateForLiquibase(10, '20160208210114').toISOString(), '2016-02-07T17:46:59.078Z');
            });
        });
    });
});
